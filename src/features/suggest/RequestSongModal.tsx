    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl w-full max-w-md text-center p-8 relative" onClick={e => e.stopPropagation()}>
                ...
                <CloudUploadIcon className="w-12 h-12 mx-auto mb-4 text-purple-500 dark:text-purple-400" />
                <h2 className="text-xl font-bold mb-2">「{songTitle}」をリクエスト</h2>
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">今後の参考にさせていただきます！</p>

                {isSent ? (
                    <div className="text-center p-6 bg-gray-100 dark:bg-gray-900 rounded-lg flex flex-col items-center gap-4">
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="mb-4">
                            <label htmlFor="casId_modal" className="block text-sm text-left font-medium text-gray-700 dark:text-gray-300 mb-1">
                                ツイキャスアカウント名 <span className="text-red-500 dark:text-red-400">*</span>
                            </label>
                            <input
                                id="casId_modal"
                                type="text"
                        ...
                        <div className="text-xs text-left text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-900 p-3 rounded-md space-y-2 mb-6">
                            <p>※リクエストに必ずお応えできるわけではありませんので、ご了承ください。</p>
                            <p>※<a href={`https://www.print-gakufu.com/search/result/keyword__${encodeURIComponent(songTitle)}/`} target="_blank" rel="noopener noreferrer" className="text-cyan-600 dark:text-cyan-400 hover:underline">「ぷりんと楽譜」<ExternalLinkIcon className="inline-block w-3 h-3"/></a>に楽譜がある曲は、初見での演奏が可能な場合があります。</p>
                        </div>

                        <button type="submit" disabled={isSending} className="w-full flex items-center justify-center gap-2 px-6 py-3 bg-purple-600 hover:bg-purple-700 text-white rounded-lg font-semibold transition-transform transform hover:scale-105 disabled:bg-gray-500 disabled:cursor-not-allowed">
